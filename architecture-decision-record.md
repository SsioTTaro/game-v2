...
**決定済み** — 2025年5月7日時点（**2026年5月7日 更新**：プラットフォームUIレイヤーの実装方式を確定）
...
| レイヤー | 技術 | 備考 |
|----------|------|------|
| フロントエンド（ゲームエンジン） | **Phaser.js** | 2D Web ゲームの定番。WebGL 描画、物理演算、Tween、パーティクル等を標準搭載。**個別ゲーム本体（Game シーン）でのみ使用** |
| プラットフォームUIレイヤー | **素の HTML / CSS / TypeScript** | Title / GameSelect / Settings / Ranking / Result はHTMLで実装。UIフレームワーク（React / Svelte / Vue）は採用しない |
...
プラットフォーム画面（Title / GameSelect / Settings / Ranking / Result）の実装方式を、当初の「全画面 Phaser シーン」案から見直し、**素の HTML / CSS / TypeScript** で実装することに確定。

**全画面 Phaser シーン（却下）**
当初は Boot / Title / GameSelect / Settings / Ranking もすべて Phaser シーンとして実装する案だった。しかしこれらはゲームプレイではなくUI主体の画面（カード一覧、フォーム、テーブル表示）。Phaser でフォーム実装やIME入力対応はハック前提（hidden な `<input>` をキャンバスに重ねる等）になり、反復速度・モバイルUX・実装コストすべてで HTML / CSS が優位。スマホ主のターゲットでプレイヤー名入力を素直に扱えることは特に重要。

**React（却下）**
AI生成コードの精度は最も高いが、4画面規模のUIに対してバンドルサイズ（45KB gz）と学習コストが釣り合わない。ボイラープレートも増える。

**Svelte 5（却下）**
ランタイムが極小・Runesによるリアクティビティ・CSSスコープ自動など、技術的にはこのプロジェクトに最も適合する選択肢。しかしオーナーが Web フレームワーク経験が浅く、Phaser 学習に集中したい時期に新フレームワーク学習を被せたくない。Jetpack Compose の経験があるためフレームワーク自体への抵抗はないが、**4画面に対して過剰**と判断。

**Vue（却下）**
Svelte 5 と React の中間。両者を上回る決定的優位がない。

**将来の選択肢として残すこと**
素TSで動かない理由が明確になった時点で、**ファイル単位で Svelte 5 に部分移行**できる構成にしておく（HTMLがそのまま Svelte コンポーネントになるため移行コストが軽い）。
...
| プラットフォームUIレイヤー | 全画面 Phaser シーン | 素のHTML/CSS/TS（殻） + Phaser（ゲーム本体のみ） | UI主体の画面はHTML/CSSの方が圧倒的に書きやすく、モバイルのIME・フォーム対応も素直。フレームワーク（React/Svelte/Vue）は4画面規模に対して過剰 |
